...
#     key_file_name = f"{password_hash}.key"
#     if key_file_name != room_data["key_file"]:
#         raise HTTPException(status_code=404, detail="Encryption key not found")

#     list_file_encrypted = []
#     list_file = []
#     chat_data = await chat_collection.find({"room_id": room_id})
#     for chat in chat_data:
#         list_file_encrypted.append(chat["encrypted_file_name"])
#         list_file.append(chat["file_name"])

#     return {
#         "room_id": room_id,
#         "key_file": key_file_name,
#         "list_file_encrypted": list_file_encrypted,
#         "list_file": list_file,
#     }
...
async def join_chatroom(
    ...
    list_file_encrypted = []
    list_file = []
    chat_cursor = chat_collection.find({"room_id": room_id})
    chat_data = await chat_cursor.to_list(length=100)
    for chat in chat_data:
        list_file_encrypted.append(chat["encrypted_file_name"])
        list_file.append(chat["file_name"])
...
async def upload_file_encrypted(
    ...
    if not os.path.exists(key_file_path):
        raise HTTPException(status_code=404, detail="Encryption key error")
